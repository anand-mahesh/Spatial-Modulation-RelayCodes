...
for snrcount=1:length(SNRdB)
    ...
    for tries = 0:NUM
        ...
        H_sd = randn(Nr,Nt) + j.*randn(Nr,Nt);
        H_sr = randn(Nr,Nt) + j.*randn(Nr,Nt);
        H_rd = randn(Nr,Nt) + j.*randn(Nr,Nt);
        snr = 10^(SNRdB(snrcount)/10); % SNR, said non-dB???dB 
        sigpwr = sum((abs(x_s)).^2)/max([mean(abs(sigConstP)) mean(abs(sigConstS))]);
        noisepwr = sigpwr./snr;
        std_dev = sqrt(noisepwr);
%         POW_S = POW_DIV;      % Signal power 
%         POW_N = POW_S / snr;  % Noise power 
...
%         a_sd = H_sd' * sqrt(POW_S)/POW_N;
%         a_rd = (beta*sqrt(POW_S)*H_sr'*H_rd')/((beta^2*(abs(H_rd))^2+1)*POW_N);
%         y_combine_AF = a_sd*y_sd + a_rd*y_rd;
        
%         [~, Idx_min_Error1] = min(sum(abs(repmat(y_rd,1,siz)-H_rd*H_sr*ESMconsdia).^2,1));
%         ML_Binary_Results1 = dec2bin(Idx_min_Error1-1,log2(siz));
%         BER_SMT_ML_RD =sum(dec2bin(x,eta)~= ML_Binary_Results1)/eta ;
%         SER_RD(snrcount) = SER_RD(snrcount) + BER_SMT_ML_RD;
...
figure
semilogy(SNRdB, SER_SD, 'mo-', 'LineWidth', 1);
hold on
semilogy(SNRdB, SER_RD,'bo-', 'LineWidth', 1);
grid on 
xlabel('Es/No, dB') 
