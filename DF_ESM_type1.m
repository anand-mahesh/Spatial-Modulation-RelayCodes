...
for snrcount=1:length(SNRdB)  
    for tries = 0:NUM
        ...
        [~, Idx_min_Error] = min(sum(abs(repmat(y_sr,1,siz)-H_sr*ESMconsdia).^2,1));
        ...
        %If error rate is less transmit else dont
        if gamma == 1
            y_rd = H_rd*y_sr + noise_rd;
        end
        
        siz = size(ESMconsdia,2);
        [~, Idx_min_Error1] = min(sum(abs(repmat(y_sd,1,siz)-H_sd*ESMconsdia).^2,1)+gamma.*sum((abs(gamma^-0.5*(repmat(y_rd,1,siz)-(alpha*H_rd*H_sr*ESMconsdia)))).^2,1));
